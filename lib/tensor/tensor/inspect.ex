defmodule Tensor.Tensor.Inspect do
  ...
  defp inspect_tensor_contents(tensor = %Tensor{dimensions: dimensions})
       when length(dimensions) == 3 do
    ...
    |> Enum.map(fn slice ->
      ...
      |> Enum.map(fn {row, index} ->
        ...
        "#{String.pad_leading("", 2 * index)}#{color(deepness, rem(index, deepness))}#{rowstr}#{
          IO.ANSI.reset()
        }"
      end)
      |> Enum.join("\n")
  ...
  defp color(deepness, depth) when deepness <= 3,
    do:
      [
        [IO.ANSI.bright(), IO.ANSI.white()],
        [IO.ANSI.white()],
        [IO.ANSI.bright(), IO.ANSI.black()]
      ]
      |> Enum.fetch!(depth)

  defp color(deepness, depth) when deepness <= 5,
    do:
      [
        [IO.ANSI.bright(), IO.ANSI.white()],
        [IO.ANSI.white()],
  ...
  defp color(deepness, depth) when deepness <= 6,
    do:
      [
        [IO.ANSI.bright(), IO.ANSI.white()],
        [IO.ANSI.white()],
